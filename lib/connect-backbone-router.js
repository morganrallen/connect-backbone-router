	...
	var methods = {
		GET: function(collection, modelId, attribute) {
			try {
				...
				if(!modelId) {
					return collections[collection].toJSON() || {};
				}

				if(!attribute || typeof attribute !== "string") {
					return collections[collection].get(modelId).toJSON();
				};
		...
		POST: function(collection, modelId, attributes) {
			...
			return collections[collection].add(attributes).toJSON();
		},

		PUT: function(collection, modelId, attributes) {
			attributes = attributes || {};
			return collections[collection].get(modelId).set(attributes).toJSON();
		},
